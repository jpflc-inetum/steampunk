...
CLASS lsc_zbp_i_online_shop_47 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_47 IMPLEMENTATION.

  METHOD save_modified.
    DATA: lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_47,
          ls_online_shop_as TYPE zshop_as_47.

    IF zbp_i_online_shop_47=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_online_shop_47=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_47 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
...
CLASS lhc_zbp_i_online_shop_47 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requeste_authorizations FOR online_shop RESULT result.

    METHODS create_pr FOR MODIFY IMPORTING keys FOR ACTION online_shop~create_pr.

    METHODS update_inforecord FOR MODIFY IMPORTING keys FOR ACTION online_shop~update_inforecord.

    METHODS calculate_order_id FOR DETERMINE ON MODIFY IMPORTING keys FOR online_shop~calculate_order_id.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_47 IMPLEMENTATION.
...
*  if a new package is ordered, trigger a new purchase requisition
      ...
      SELECT SINGLE * FROM zi_online_shop_prepitem_xxx
        WHERE Pkgid = @<key>-%param-PackageId INTO @DATA(ls_prepitem).
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( ( CostCenter   = <key>-%param-CostCenter " e.g. 'JMW-COST'
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API 47'
                                        ) (
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API 47'
                                        ) )
              )   )
      ...
      zbp_i_online_shop_47=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    ...
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_47,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_47.

    SELECT MAX( order_id ) FROM zonlineshop_47 INTO @DATA(max_order_id).

    READ ENTITIES OF zi_online_shop_47 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_online_shop_47 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_47~create_pr,
